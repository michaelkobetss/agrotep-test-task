...
async function fetchDataAndFillTable() {
    try {
      ...
      // Call editTable again to update the table for editing
    //   editTable();
    } catch (error) {
...
function editTable() {
  ...
  for (let i = 0; i < table.rows[0].cells.length; i++) {
    ...
    button.innerHTML =
      '<i class="fas fa-trash"></i><i class="fas fa-left-right"></i>';
...
function finishEditing() {
  ...
  // Iterate over all rows in the table, starting from the fourth row
    ...
    .map((row) => {
      ...
      Array.from(row.cells)
        ...
        .forEach((cell, index) => {
          ...
          const tHeader = cell.querySelector("th")
